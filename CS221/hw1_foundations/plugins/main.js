...
function onLoad(assignmentId, ownerName, version, edLink) {
  ...
  ol.append($('<li>').addClass('writeup').addClass('template').attr('data-icon', 'Written submission required').append('This icon means you should write responses in <code>' + assignmentId + '.pdf</code>.'));
  ol.append($('<li>').addClass('code').addClass('template').attr('data-icon', 'Code submission required').append('This icon means you should write code in <code>submission.py</code>.'));
  header.append(ol);
  ...
  header.append($('<p>').append(
    ...
    'To run the tests, you will need to have <code>grader_util.py</code> in the same directory as your code and <code>grader.py</code>. ' +
