...
class GhostRules:
    ...
    def apply_action(state, action, ghost_index):
        ...
        ghost_state.configuration = ghost_state.configuration.generate_successor(
            vector)
    apply_action = staticmethod(apply_action)
...
def read_command(argv):
  ...
  """
    ...
    parser.add_option('-m', '--terminal_graphics', action='store_true', dest='terminal_graphics',
                      help='Display output as terminal_graphics', default=False)
    parser.add_option('-q', '--quiet_text_graphics', action='store_true', dest='quiet_graphics',
                      help='Generate minimal output and no graphics', default=False)
    parser.add_option('-g', '--ghosts', dest='ghost',
                      help=default(
                          'the ghost agent TYPE in the ghost_agents module to use'),
    ...
    if options.terminal_graphics and options.pacman == 'KeyboardAgent':
        import keyboard_agents
        keyboard_agents.USE_CURSE = True
    ...
    elif options.terminal_graphics:
        import text_display
        text_display.SLEEP_TIME = options.frame_time
        args['display'] = text_display.CurseDisplay()
    else:
...
def load_agent(pacman, nographics):
    ...
    for module_dir in python_path_dirs:
        if not os.path.isdir(module_dir):
            continue
        module_names = [f for f in os.listdir(module_dir) if f.endswith(
            'gents.py') or f == 'submission.py']
...
def replay_game(layout, actions, display):
    ...
    agents = [submission.ExpectimaxAgent()] + [ghost_agents.RandomGhost(i+1)
