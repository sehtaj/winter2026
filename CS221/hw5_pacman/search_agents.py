...
> python pacman.py -p SearchAgent -a search_function=depth_first_search
...
class SearchAgent(Agent):
    ...
    def __init__(self, fn='depth_first_search', prob='PositionSearchProblem', heuristic='null_heuristic'):
        ...
        if 'heuristic' not in func.__code__.co_varnames:
            ...
            if heuristic in dir(search_agents):
                heur = getattr(search_agents, heuristic)
        ...
        if prob not in dir(search_agents) or not prob.endswith('Problem'):
            raise AttributeError(
                prob + ' is not a search problem type in SearchAgents.py.')
        self.search_type = getattr(search_agents, prob)
    ...
    def get_action(self, state):
        """
        Returns the next action in the path chosen earlier (in register_initial_state).  Return
...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self, game_state, cost_fn=lambda x: 1, goal=(1, 1), start=None, warn=True):
        """
...
class StayEastSearchAgent(SearchAgent):
    ...
    def __init__(self):
        ...
        def cost_fn(pos): return .5 ** pos[0]
...
def food_heuristic(state, problem):
    ...
    The state is a tuple (pacman_position, food_grid) where food_grid is a 
    Grid (see game.py) of either True or False. You can call food_grid.as_list()
    ...
    there is a dictionary called problem.heuristic_info that you can use. For example,
    if you only want to count the walls once and store that value, try:
      problem.heuristic_info['wall_count'] = problem.walls.count()
    Subsequent calls to this heuristic can access problem.heuristic_info['wall_count']
    """
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def register_initial_state(self, state):
        ...
        while (current_state.get_food().count() > 0):
            next_path_segment = self.find_path_to_closest_dot(
                current_state)  # The missing piece
    ...
    def find_path_to_closest_dot(self, game_state):
        "Returns a path (a list of actions) to the closest dot, starting from game_state"
...
class AnyFoodSearchProblem(PositionSearchProblem):
    """
      ...
      the find_path_to_closest_dot method.
    """
...
def maze_distance(point1, point2, game_state):
    ...
    you have already built.  The game_state can be any game state -- Pacman's position
    ...
    prob = PositionSearchProblem(
        game_state, start=point1, goal=point2, warn=False)
