...
class Grid:
    ...
    def __init__(self, width, height, initial_value=False, bit_representation=None):
        ...
        self.width = width
        self.height = height
        self.data = [[initial_value for y in range(
    ...
    def __str__(self):
        ...
        for agent_state in self.agent_states:
            ...
            if agent_state.configuration is None:
                continue
            x, y = [int(i)
                    for i in nearest_point(agent_state.configuration.pos)]
    ...
    def run(self):
        ...
        for i in range(len(self.agents)):
            ...
            if ("register_initial_state" in dir(agent)):
                ...
                if self.catch_exceptions:
                    try:
                        timed_func = TimeoutFunction(agent.register_initial_state, int(
                            self.rules.get_max_startup_time(i)))
                        try:
        ...
        while not self.game_over:
            ...
            if 'observation_function' in dir(agent):
                ...
                if self.catch_exceptions:
                    try:
                        timed_func = TimeoutFunction(agent.observation_function, int(
            ...
            if self.catch_exceptions:
                try:
                    timed_func = TimeoutFunction(agent.get_action, int(
                        self.rules.get_move_timeout(agent_index)) - int(move_time))
                    try:
                        start_time = time.time()
                        if skip_action:
                        ...
                        print(("Agent %d timed out on a single move!" % agent_index))
                        self.agent_timeout = True
                        self.unmute()
                        self._agent_crash(agent_index, quiet=True)
                        return
