...
Usage (see grader.py):
    ...
    # add a basic test
    grader.add_basic_part(number, grade_func, max_points, max_seconds, description="a basic test")

    # add a hidden test
    grader.add_hidden_part(number, grade_func, max_points, max_seconds, description="a hidden test")

    # add a manual grading part
    grader.add_manual_part(number, grade_func, max_points, description="written problem")
...
def is_traceback_item_grader(item):
    return item[0].endswith('grader_util.py')
...
class Grader:
    def __init__(self, args=None):
        ...
        self.fatal_error = False  # Set this if we should just stop immediately
    ...
    def grade(self):
        ...
        # Grade it!
        if not self.params.summary and not self.fatal_error:
            print('========== START GRADING')
            for part in parts:
